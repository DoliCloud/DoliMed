...
if (isModEnabled("fournisseur") && ! empty($user->rights->fournisseur->lire)) {
	...
	if ((!$object->code_fournisseur || $object->code_fournisseur == -1) && $modCodeFournisseur->code_auto) {
		...
		print '<input type="text" name="code_fournisseur" id="supplier_code" size="16" value="'.dol_escape_htmltag($tmpcode).'" maxlength="24">';
	} elseif ($object->codefournisseur_modifiable()) {
		print '<input type="text" name="code_fournisseur" id="supplier_code" size="16" value="'.$object->code_fournisseur.'" maxlength="24">';
	} else {
...
// EMail / Web
print '<tr><td>';
print empty($conf->global->SOCIETE_EMAIL_MANDATORY) ? '' : '<span class="fieldrequired">';
print $langs->trans('EMail');
print empty($conf->global->SOCIETE_EMAIL_MANDATORY) ? '' : '</span>';
print '</td><td colspan="3"><input type="text" name="email" size="32" value="'.$object->email.'"></td>';
print '</tr>';
